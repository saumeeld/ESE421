...
ranges=(0,5,31) # Range of acceptable HSV values
...
# Hard-coded thresholds for orange (Adnan)
lowerthres = np.array([0,200,170], dtype=np.uint8)
upperthres = np.array([30,255,255],dtype=np.uint8)

mask = cv2.inRange(imghsv, lowerthres, upperthres)
## END RYAN'S CODE

# Perform Canny edge detection on selected region of interest
# (No offset for platoon image)
